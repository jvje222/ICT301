...
if (isset($_POST['submitted'])) {
  ...
  if (empty($_POST['email'])) {
    $erros[] = 'You forgot to enter your email address.';
  } else {
    $email = mysqli_real_escape_string($dbc, $_POST['email']);
  }
  if (!empty($_POST['password1']) && !empty($_POST['password2'])) {
    $erros[] = 'You password did not match the confirmed password.';
  } else {
...
if (empty($errors)) {
  ...
  if (mysqli_num_rows($result) ==0) {
    $query = "INSERT INTO users (first_name, last_name, emial, password, registration_date) VALUES ('$first_name', '$last_name', '$email', SHA2('$password', 512), NOW() )";
    $result = @mysqli_query ($dbc, $query);
    if ($result) {
      echo "<p>You could not be registered. Please, login to our website.</p>";
      echo "<a herf=index.php>Login</a>";
      mysqli_close($dbc);
      exit();
    } else {
      $errors[] = 'You could not be registered due to system error We apologize for any inconvenience.';
